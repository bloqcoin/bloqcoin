...
Run with the -testnet option to run with "play bloqcoins" on the test network, if you
...
Bloqcoin is a multithreaded application, and deadlocks or other multithreading bugs
...
- BloqcoinMiner : Generates bloqcoins (if wallet is enabled).
...
However, in open source software such as Bloqcoin, where everyone uses
...
pay attention to for reviewers of Bloqcoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Bloqcoin uses tinyformat, which is type safe. Leave them out to avoid confusion
