...
#if defined(NDEBUG)
# error "Bloqcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Bloqcoin Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BLOQ additional fees, %d delta bytes\n",
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex || chainActive.Height() < Checkpoints::GetTotalBlocksEstimate(chainParams.Checkpoints()))
        return true;
    return (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 || pindexBestHeader->GetBlockTime() < GetTime() - 24 * 60 * 60);
}
...
void ThreadScriptCheck() {
    RenameThread("bloqcoin-scriptch");
